...
import Control, {getTitle, makeString} from '../Control/Control';
...
const BTN_TYPE = ['small', 'default', 'large'];
...
class Search2 extends React.Component {
  static propTypes = {
    ...
    defaultSize: PropTypes.oneOf(BTN_TYPE),
  };
  ...
  toButton = (key, props={}) => {
    const {config, defaultSize='small'} = this.props;
  ...
  toButtons = () => {
    const noTitle = this.props.filters.find(o => !o.title) ? s.noTitle : '';
    return (
      <div role='buttons' className={noTitle}>
  ...
  getControlProps = (control) => {
    const {data={}, defaultSize='small'} = this.props;
    return {
      ...(control.props || {}),
      size: defaultSize,
      type: control.type,
