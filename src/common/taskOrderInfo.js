...
const buildTaskOrderDetailState = async (guid) => {
  ...
  for (let item of orderList) {
    ...
    const title = `${jobTitle}${taskUnitNumber} (${taskUnitInfo.taskUnitTypeName}/${taskUnitStatus})   客户委托号：${mainOrderInfo.baseInfo.customerDelegateCode || ''}
    委托客户：${mainOrderInfo.baseInfo.customerGuid.title || ''}`;
