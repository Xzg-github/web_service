...
import showShareDialog from './ShareDialogContainer'


const STATE_PATH = ['enterprise_documents'];
const action = new Action(STATE_PATH);

const URL_LIST = '/api/signature/enterprise_documents/list';
const URL_CONFIG = '/api/signature/enterprise_documents/config';
const URL_TREE_LIST = '/api/signature/enterprise_documents/tree';
...
//共享文件夹
const shareAction = () =>  async (dispatch, getState) => {
  const {shareConfig} = getSelfState(getState());
  if (await showShareDialog(shareConfig,[])) {
  }
};
//移动文件
