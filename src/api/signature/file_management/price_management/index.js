...
api.post('/list', async (req, res) => {
  ...
  const url = `${host}/price_manager/list`;
  res.send(await fetchJsonByNode(req,url,postOption(body)))
});

//新增
api.post('/update', async(req,res) => {
  const url = `${host}/price_manager`;
...
api.delete('/del', async(req,res) => {
  const url = `${host}/price_manager/batch`;
  res.send(await fetchJsonByNode(req,url,postOption(req.body,'delete')))
});

//业务项目的下拉
api.post('/dropDown', async(req,res) => {
  const url = `${host}/business_items/drop_list`;
  res.send(await fetchJsonByNode(req,url,postOption(req.body)))
});

//批量失效
api.put('/disable/:statusType', async(req,res) => {
  const url = `${host}/price_manager/batch/${req.params.statusType}`;
  res.send(await fetchJsonByNode(req,url,postOption(req.body,'put')))
});

//获取单条
api.get('/one/:id', async(req,res) => {
  const url = `${host}/price_manager/${req.params.id}`;
